...
# CSS Modifications
# Add card-bg variable to root
css_content = css_content.replace(
    '--text-light: #FFFFFF;',
    '--text-light: #FFFFFF;\n  --card-bg: #FFFFFF;'
)

# Use card-bg instead of text-light for objects
css_content = css_content.replace('background-color: var(--text-light)', 'background-color: var(--card-bg)')
css_content = css_content.replace('background: var(--text-light)', 'background: var(--card-bg)')

# Dark theme overrides & Premium features CSS
premium_css = """
/* =========================================
...
const premiumEnhancements = () => {
    ...
    window.addEventListener('mousemove', (e) => {
        ...
        const now = Date.now();
        if (now - lastSparkleTime > 40) { // spawn faster for smoother trail
...
// Inject execution call before DOMContentLoaded ends inside original JS
"""

js_content = js_content.replace('});\n', '\\n' + premium_js + '\\n    premiumEnhancements();\\n});\n')

# Actually, the original js might just have `});` at the end cleanly!
# To do it safer without exact matching `});\\n`:
with open(js_path, 'w', encoding='utf-8') as f:
    f.write(premium_js + "\\n\\n" + js_content.replace('});', '    premiumEnhancements();\\n});'))

print("Modifications successfully injected!")
